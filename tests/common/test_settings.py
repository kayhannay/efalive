...
class EfaLiveSettingsTestCase(unittest.TestCase):
    ...
    def test_init__path_exists(self):
        ...
        self.assertEqual(None, settings.mailer_sender.getData())
    ...
    def test_save__settings_filled(self, open_mock):
        ...
        settings.mailer_sender.updateData("User <user@test.local>")
        settings.hourly_tasks.updateData({"123" : ["SHELL", "ls /tmp0"]})
        settings.daily_tasks.updateData({"234" : ["BACKUP", ""], "345" : ["SHELL", "ls /tmp1"]})
        settings.weekly_tasks.updateData({"456" : ["SHELL", "ls /tmp2"]})
        settings.monthly_tasks.updateData({"567" : ["SHELL", "ls /tmp3"]})
        settings.save()
    ...
    def __init__(self, is_empty):
        if is_empty:
            ...
            self.settings_list = [
                    ...
                    "MAILER_SENDER='User <user@test.local>'",
