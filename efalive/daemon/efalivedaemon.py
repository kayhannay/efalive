        ...
        while True:
            try:
                ...
                self._logger.exception(
                    "An error occured which was not handled in it's sub module:"
                )
    ...
    def run_checks(self):
        ...
        if process_count < 1:
            ...
            self._logger.warning(
                "Process '%s' is not running, wait for %d more checks before restart!"
                % (process_name, self._restart_threshold)
            )
            ...
            self._logger.debug(
                "Found %d instances of the process '%s'."
...
class AutoBackupModule(object):
    ...
    def _handle_usb_add_event(self, device):
        self._logger.info(
            "USB storage device added: [%s] %s %s (%s, %s)"
            % (
                device.bus_id,
                device.vendor,
                device.model,
    ...
    def _run_autobackup(self, device_file):
        try:
            (returncode, output) = common.command_output(
                ["/usr/lib/efalive/bin/autobackup.sh", device_file]
            )
            if returncode != 0:
...
class TaskSchedulerModule(UpdateableModule):
    ...
    def update_settings(self, settings):
        ...
        self.hourly_tasks = self._create_task_list(
            self._settings.hourly_tasks.getData()
        )
        self.daily_tasks = self._create_task_list(self._settings.daily_tasks.getData())
        self.weekly_tasks = self._create_task_list(
            self._settings.weekly_tasks.getData()
        )
        self.monthly_tasks = self._create_task_list(
            self._settings.monthly_tasks.getData()
        )
    ...
    def _create_task_list(self, tasks):
        ...
        for task_id in tasks.keys():
            if tasks[task_id][0] == "SHELL":
                ...
                backupMailTask = BackupMailTask(
                    task_id, tasks[task_id][1], self._settings
                )
