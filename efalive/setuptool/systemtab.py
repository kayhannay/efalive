...
class SystemTabView(Gtk.VBox):
    ...
    def _init_components(self):
        ...
        self.systemGrid.attach(self.networkButton, 0, 1, 0, 1)
        ...
        self.systemGrid.attach(self.dyndnsButton, 1, 2, 0, 1)
        ...
        self.systemGrid.attach(self.powerManagerButton, 2, 3, 0, 1)
        ...
        self.systemGrid.attach(self.datetimeButton, 0, 1, 1, 2)
        ...
        self.systemGrid.attach(self.keyboardButton, 1, 2, 1, 2)
...
class SystemTabController(object):
    ...
    def runNetworkSettings(self, widget):
        try:
            ...
            dialogs.show_exception_dialog(
                self._view.get_toplevel(), message, traceback.format_exc()
            )

    def runKeyboardSetup(self, widget):
        try:
            subprocess.Popen(
                ["foot", "sudo", "dpkg-reconfigure", "keyboard-configuration"]
            )
        except OSError as error:
    ...
    def runDyndnsSetup(self, widget):
        try:
            subprocess.Popen(["foot", "sudo", "dpkg-reconfigure", "ddclient"])
        except OSError as error:
    ...
    def runPowerManagerSetup(self, widget):
        try:
            ...
            message = (
                _("Could not open xfce4-power-manager-settings program: %s") % error
            )
            self._logger.error(message)
            dialogs.show_exception_dialog(
                self._view.get_toplevel(), message, traceback.format_exc()
    ...
    def run_raspi_config(self, widget):
        try:
            subprocess.Popen(["foot", "sudo", "raspi-config"])
        except OSError as error:
